...
function parseInput(input) {
    ...
    const tiles = new Array(h).fill(null).map(() => new Array(w).fill(Tile.Air));
    for (const rockGroup of rockGroups) {
        ...
        for (let i = 1, len = rockGroup.length; i < len; i++) {
            ...
            if (prevX !== currX) {
                ...
                for (let sx = Math.min(prevX, currX), ex = 1 + Math.max(prevX, currX); sx < ex; sx++) {
            ...
            // vertical
                ...
                if (prevX !== currX)
                    throw new Error('Logic Error');
                for (let sy = Math.min(prevY, currY), ey = 1 + Math.max(prevY, currY); sy < ey; sy++) {
...
export function solve(input, hasBounds) {
...
export const partOne = (input) => solve(input, true);
