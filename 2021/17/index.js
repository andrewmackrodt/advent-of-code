...
export function getMaxY(input) {
    const velocities = getVelocities(input).sort((a, b) => b.ySpeed - a.ySpeed);
    return velocities[0].maxHeight;
}
export function getUniqueValidVelocityCount(input) {
    return getVelocities(input).length;
}
export const partOne = (input) => getMaxY(input);
export const partTwo = (input) => getUniqueValidVelocityCount(input);
